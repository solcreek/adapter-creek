...
export function generateWorkerEntry(opts: WorkerEntryOptions): string {
  ...
  // Static imports — ensures wrangler bundles ALL handler modules and
  // their Turbopack chunks. Dynamic import() may not be followed.
  const handlerImports = handlers
    .map((h) => `import * as ${h.varName} from ${JSON.stringify(h.importPath)};`)
    .join("\n");

  const handlerEntries = handlers
    .map(
      (h) =>
        `  ${JSON.stringify(h.pathname)}: { mod: ${h.varName}, runtime: ${JSON.stringify(h.runtime)}, type: ${JSON.stringify(h.type)} },`,
    )
    .join("\n");
...
// Static handler imports — wrangler follows these and bundles all chunks
${handlerImports}
