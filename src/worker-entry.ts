  ...
  const response = await __INTERNAL_FETCH_CONTEXT.run({ origin: new URL(request.url).origin, env, ctx, request }, async () => {
    try {
      ...
      if (result.externalRewrite) {
        ...
        // For App Router RSC requests whose \`externalRewrite\` is actually a
        // same-origin middleware rewrite (different hostname due to
        // request.headers.host vs worker URL), surface the rewrite target
        // via \`x-middleware-rewrite\` / \`x-nextjs-rewritten-path\` /
        // \`x-nextjs-rewritten-query\` so the client router can key its
        // prefetch cache by the REWRITTEN URL instead of the original.
        // Matches vanilla Next's behavior for prefetch responses from
        // routes with middleware rewrites. Required for
        // segment-cache/search-params "stores prefetched data by its
        // rewritten search params" / "handles rewrites to the same page
        // but with different search params".
        try {
          const isAppRSC =
          ...
          if (isAppRSC && !cleanHeaders.has("x-middleware-rewrite")) {
            ...
            if (origSearch !== newSearch && !cleanHeaders.has("x-nextjs-rewritten-query")) {
              cleanHeaders.set(
                ...
                newSearch.startsWith("?") ? newSearch.slice(1) : newSearch
              );
            }
            if (url.pathname !== rewriteUrlObj.pathname && !cleanHeaders.has("x-nextjs-rewritten-path")) {
              cleanHeaders.set("x-nextjs-rewritten-path", rewriteUrlObj.pathname);
            }
          }
        } catch {}
      ...
      // Prefetch-RSC 404 fast-path for cacheComponents routes without a
      // build-time prefetch shell. When a route in prerender-manifest is
      // marked \`renderingMode: "PARTIALLY_STATIC"\` but has
      // \`prefetchDataRoute: null\`, Next.js deliberately returns 404 for
      // prefetch RSC requests — the signal tells the client router
      // "no cached shell here; either use the rewrite hint or fetch the
      // full RSC on navigation". Without this, our adapter renders the
      // full page for the prefetch, so the client caches the full content
      // under the original URL instead of learning about the middleware
      // rewrite. That breaks \`segment-cache/search-params\` tests which
      // expect prefetched data to be keyed by the REWRITTEN search
      // params (so two different original URLs that rewrite to the same
      // target share one cache entry and the second reveal doesn't
      // re-fetch). Middleware rewrite info (\`result.mwRewrite\`,
      // \`resolvedHeaders\`) is captured before this point and propagated
      // on the 404 response so the client can act on it.
      if (
        isAppRouterRSCRequest &&
        ...
        try {
          const pm = __getPrerenderManifest();
          const manifestRoute =
            (pm?.routes && resolvedPathname && pm.routes[resolvedPathname]) ||
            (pm?.dynamicRoutes && resolvedPathname && pm.dynamicRoutes[resolvedPathname]) ||
            null;
          if (
            manifestRoute &&
            manifestRoute.renderingMode === "PARTIALLY_STATIC" &&
            (manifestRoute.prefetchDataRoute == null ||
              manifestRoute.prefetchDataRoute === "")
          ) {
            const headers = new Headers();
            headers.set("Vary", "rsc, next-router-state-tree, next-router-prefetch, next-router-segment-prefetch");
            headers.set("x-nextjs-cache", "HIT");
            headers.set("x-nextjs-prerender", "1");
            headers.set("x-nextjs-postponed", "1");
            headers.set("x-nextjs-stale-time", "300");
            headers.set("Cache-Control", "s-maxage=31536000");
            headers.set("x-nextjs-deployment-id", DEPLOYMENT_ID);
            // Forward middleware rewrite info so the client router can
            // de-duplicate prefetch cache entries by the rewritten URL.
            if (result.mwRewrite) {
              headers.set("x-middleware-rewrite", result.mwRewrite);
              try {
                const rewriteUrl = new URL(result.mwRewrite, url.origin);
                const origSearch = url.search || "";
                const newSearch = rewriteUrl.search || "";
                if (origSearch !== newSearch) {
                  // Next.js strips the leading "?" from this header.
                  headers.set(
                    "x-nextjs-rewritten-query",
                    newSearch.startsWith("?") ? newSearch.slice(1) : newSearch
                  );
                }
                if (url.pathname !== rewriteUrl.pathname) {
                  headers.set("x-nextjs-rewritten-path", rewriteUrl.pathname);
                }
              } catch {}
            }
            if (result.resolvedHeaders) {
              result.resolvedHeaders.forEach((val, key) => {
                const k = key.toLowerCase();
                if (k === "set-cookie") headers.append(key, val);
                else if (!headers.has(key)) headers.set(key, val);
              });
            }
