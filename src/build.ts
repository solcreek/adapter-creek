...
export async function handleBuild(ctx: BuildContext): Promise<void> {
  ...
  // Phase 1 (experimental): emit a 3-worker scaffold alongside the
  // single-worker bundle when \`CREEK_MULTI_WORKER=1\` is set. This
  // proves the build pipeline can produce dispatcher + node-runtime +
  // edge-runtime bundles that a miniflare SDK launcher can host with
  // service bindings. Real handler splitting arrives in Phase 2.
  if (process.env.CREEK_MULTI_WORKER === "1") {
    await emitMultiWorkerScaffold(ctx, outputDir);
  }
...
/**
 * Write a minimal 3-worker scaffold (dispatcher + node-runtime +
 * edge-runtime). No real handlers yet — each worker just echoes its
 * origin. The launcher at \`scripts/multi-worker-dev.mjs\` wires them
 * together so an end-to-end fetch can prove the topology.
 *
 * Runs ONLY in experimental mode. Does not alter the single-worker
 * bundle that real deploys still use.
 */
async function emitMultiWorkerScaffold(
  ctx: BuildContext,
  outputDir: string,
): Promise<void> {
  const runtimes = classifyHandlersByRuntime(ctx.outputs);
  console.log(
    `  [Creek Adapter] Multi-worker scaffold: nodejs=${runtimes.nodejs} edge=${runtimes.edge}`,
  );
  ...
  // Dispatcher: routes requests by URL prefix for now. Phase 2 will
  // replace this with middleware + @next/routing + runtime-label
  // dispatch.
  const dispatcherSource = `// Creek adapter — dispatcher worker (scaffold).
// Generated by handleBuild/emitMultiWorkerScaffold. Phase 2 will
// replace the prefix-based routing with real middleware + resolveRoutes.
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    if (url.pathname.startsWith("/__edge/")) {
      return env.EDGE_WORKER.fetch(request);
    }
    if (url.pathname.startsWith("/__node/")) {
      return env.NODE_WORKER.fetch(request);
    }
    return new Response(JSON.stringify({
      from: "dispatcher",
      path: url.pathname,
      hint: "prefix with /__edge/ or /__node/ to reach runtime workers",
...
`;
  ...
  const runtimeScaffold = (label: string) => `// Creek adapter — ${label}-runtime worker (scaffold).
// Phase 2 will replace this with the runtime-specific handler bundle.
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    return new Response(JSON.stringify({
      ...
      headers: {
        ...
        "x-creek-worker-origin": "${label}",
      },
    });
  },
};
`;
  await fs.writeFile(path.join(nodeDir, "worker.js"), runtimeScaffold("node"));
  await fs.writeFile(path.join(edgeDir, "worker.js"), runtimeScaffold("edge"));

  // Each runtime worker needs its own wrangler.toml — mostly a stub
  // for deploy parity. Dispatcher gets the service bindings. DO class
  // declarations follow in Phase 2 once we move cache handlers.
  await fs.writeFile(
    path.join(dispatcherDir, "wrangler.toml"),
    [
      "# Generated by adapter-creek (multi-worker scaffold, Phase 1).",
  ...
  await fs.writeFile(
    path.join(nodeDir, "wrangler.toml"),
    [
      "# Generated by adapter-creek (multi-worker scaffold, Phase 1).",
  ...
  console.log(
    `  [Creek Adapter] Multi-worker scaffold: dispatcher/ node-runtime/ edge-runtime/ emitted`,
  );
}
